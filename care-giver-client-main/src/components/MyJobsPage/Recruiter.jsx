...
const Recruiter = () => {
    const {
        ...
        queryFn: async () => {
            const response = await axios.get(
                `http://localhost:9000/api/v1/application/recruiter-jobs`,
                {
    ...
    const handleAcceptStatus = (id, recruiterId) => {
        ...
        updateJobStatusMutation.mutate({
            body: newStatus,
            url: `http://localhost:9000/api/v1/application/${id}`,
        });
    };
                        ...
                        {jobs?.map((job, index) => {
                            ...
                            return (
                                <tr key={job?._id}>
                                    ...
                                    <td>{job?.jobId?.jobLocation}</td>
                                    <td>{job?.jobId?.jobType}</td>
                                    <td>{job?.jobId?.jobSalary}</td>
                                    <td>{job?.status}</td>
